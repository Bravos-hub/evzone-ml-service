...
from src.utils.metrics import prediction_requests
...
class PredictionService:
    ...
    async def predict_failure(
        ...
        try:
            ...
            cached = await self.cache_service.get_prediction(
                "failure", charger_id, tenant_id=tenant_id, metrics=metrics
            )
            if cached:
            ...
            result = await asyncio.to_thread(model.predict, metrics, tenant_id=tenant_id)
            ...
            await self.cache_service.set_prediction(
                "failure", charger_id, result, tenant_id=tenant_id, metrics=metrics
            )
    ...
    async def predict_maintenance(
        ...
        try:
            # Check cache first
            cached = await self.cache_service.get_prediction(
                "maintenance", charger_id, tenant_id=tenant_id, metrics=metrics
            )
            if cached:
            ...
            if isinstance(predicted_date, str):
                try:
                    failure_pred["predicted_failure_date"] = datetime.fromisoformat(
            ...
            result = await asyncio.to_thread(
                optimizer.recommend, metrics, failure_pred, tenant_id=tenant_id
            )
            result["timestamp"] = datetime.utcnow().isoformat()
            if tenant_id:
                result["tenant_id"] = tenant_id
            ...
            # Cache result
            await self.cache_service.set_prediction(
                "maintenance", charger_id, result, tenant_id=tenant_id, metrics=metrics
            )
            ...
            if result.get("urgency") == "CRITICAL":
                ...
                await self.kafka_producer.publish(
                    settings.kafka_topic_failure_alerts, {"type": "MAINTENANCE_CRITICAL", **result}
                )
    ...
    async def detect_anomaly(
        ...
        try:
            ...
            # Detect anomaly
            result = await asyncio.to_thread(detector.detect, metrics, tenant_id=tenant_id)
            result["timestamp"] = datetime.utcnow().isoformat()
            if tenant_id:
                result["tenant_id"] = tenant_id
